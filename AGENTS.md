...
- [x] **v6.8 扫参分析** ✅ — 11,622 组 (α_f=30-70), 最优 α_f=70°/α_b=15°, L/W=3.89, 详见 DESIGN_v68
- [x] **v6.8 扩扫 α_f=30-70** ✅ — 粗网格 1,296 组完成, α_b 最优从 3° 反转到 15°
...
### 当前表现 (k_clap=0.5, L/W=3.89, peak_θ=27.9°) — v6.8 α_f=30-70 扫参后
...
### 设计参数 (v6.8) ⭐ UPDATED — α_f 扩展扫描到 70°

v6.8 扫参基于**速度耦合 clap-fling** (Lighthill 公式启发)。
粗网格补扫 α_f=[30,40,60,70] (1,296 组) + 旧细网格 α_f=[50,55] (10,326 组，已迁移动硬盘)。

**关键发现**: L/W 随 α_f 单调递增 (30→70)，α_b 最优从 3° (低 α_f) 反转到 15° (高 α_f)。

```python
DESIGN_v68 = {
    # 气动安装角
    "alpha_front_deg": 70,    # 前翅安装角 (α_f↑ L/W↑, 70° 最优, 仍在 Dickinson 有效区)
    "alpha_back_deg": 15,     # 后翅安装角 (高 α_f 需高 α_b 维持俯仰平衡)
    # 翅膀相位
    "phase_diff_deg": -30,    # 前后翅相位差 (-30° L/W 最高, θ<30°)
    # 机构参数
    "mech_a": 6.0,            # 曲柄半径 [mm] — a=6 >> a=8 一致
    "mech_R": 2.50,           # 摇杆半径 [mm] — R=2.50 >> R=2.25
    "phi_offset_deg": -25,    # 翅膀安装偏角
    # 拍动
    "f": 17,                  # 拍动频率 [Hz]
    # 阻尼
    "c_damp": 5e-4,           # 俯仰阻尼
    "rotation": "cw",         # 曲柄转向
    # Clap-fling
    "k_clap": 0.5,            # k_max (0.3 和 0.5 几乎并列, 0.5 L/W 略高)
}
# L/W=3.892, peak_θ=27.9°, Fz_world=+747mN, n_exceed_90=0

# R=2.25 机构约束方案:
DESIGN_v68_R225 = {
    "alpha_front_deg": 70, "alpha_back_deg": 15, "phase_diff_deg": -30,
    "mech_a": 6.0, "mech_R": 2.25, "phi_offset_deg": -25,
    "f": 17, "c_damp": 5e-4, "rotation": "cw", "k_clap": 0.5,
}
# L/W=2.789, peak_θ=36.2°, Fz_world=+535mN (R=2.50 比 R=2.25 高 40% L/W)
```

#### v6.8 扫参关键发现 (完整版 — α_f=30-70 全覆盖)
...
##### 与 DESIGN_v67 对比
| 指标 | v6.7 | v6.8 (R=2.50) | v6.8 (R=2.25) |
|------|------|----------------|----------------|
| α_f | 60° | **70°** | 70° |
| α_b | 10° | **15°** | 15° |
| ph | -20° | **-30°** | -30° |
| R | 2.25mm | **2.50mm** | 2.25mm |
| k_clap | 0.5 | 0.5 | 0.5 |
| L/W | 2.15 | **3.89** | 2.79 |
| peak_θ | 37.6° | **27.9°** | 36.2° |
