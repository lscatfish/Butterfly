...
> 4. **上拍升力方向反转**：由于攻角固定，上拍时相对气流方向反转，平动升力方向变为**向下**（负升力）。动态分析中通过 `sign_C_L = np.sign(u·n_up)` 实现方向感知。
> 5. **旋转力消失**：固定攻角意味着 `α̇ = 0`，旋转力项为零。
> 6. 旋转力和附加质量力的公式为简化峰值估算，未做严格的翼型截面积分。
...
|------|----------------|-------------------|
| 重量 | 245 mN | 245 mN |
| 峰值升力 | ~78,000 mN | ~87,000 mN |
| 时均升力 | ~21,500 mN | ~33,800 mN |
| **时均阻力** | **~4,100 mN** | **~7,300 mN** |
| 升重比 | ~88 | ~138 |

> **为什么静态估算（5,000 mN）远低于动态分析（21,500 mN）？**
> - 静态估算只做简化 `峰值/2`，假设半个周期为正升力、半个周期为零；
> - 动态分析完整积分：下拍产生强正升力，上拍产生负升力（向下），但下拍幅度更大（80° vs 60°），且附加质量力在 stroke reversal 附近有向上分量，净结果仍为正；
> - 静态估算未计入附加质量力的时均贡献，也未反映上拍负升力的抵消效应。
>
> **注意**：固定攻角模型的时均升力仍远高于重量，原因包括：
> 1. 下拍幅度（80°）大于上拍（60°），下拍产生正升力的时间/幅度占优；
> 2. 下拍时平动升力 + 附加质量力均向上，而上拍时只有平动升力向下（附加质量力在 stroke reversal 附近仍有向上分量）；
> 3. 准定常模型未计入三维效应和涡脱落造成的升力损失。
>
...
2. ~~**攻角翻转模型**：在 stroke reversal（φ 极值点）处平滑过渡，使用数值梯度计算 α̇，已修复周期边界连续性。~~（已移除：机械蝴蝶采用刚性安装，攻角固定为 +45°，无主动翻转）
3. **参数扫描**：频率 10-25 Hz、幅度 60-140°、攻角 20-70°，输出阻力/升力随参数变化曲线。
4. **力 vs 转角图**：`force_vs_phi.png`，x 轴为翅膀转角 φ，**约定：向上为正，向下为负**。
5. **Markdown 报告**：自动生成 `气动分析报告.md`，包含参数表、结果表、图表引用、SolidWorks 导出步骤。

### 转角约定（重要）
- `φ = -Φ·sin(ωt)`：**向上为正（+），向下为负（-）**
- `φ < 0`：下拍（downstroke，翅膀向下运动）
- `φ > 0`：上拍（upstroke，翅膀向上运动）
- ~~攻角翻转发生在 φ 极值点（stroke reversal）~~（固定攻角，无翻转）
