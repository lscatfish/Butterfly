...
- 翻转角速度：`α̇_max = (π/2) / (flip_ratio·T/2)`，其中 `flip_ratio = 0.08`（翻转过渡区占半周期 8%）
...
| 旋转力 | `ρ·C_rot·α̇·φ̇·c_avg²·R²·r̂₁` | 峰值 × 2·flip_ratio（翻转占全周期 16%） |
...
> 3. 攻角翻转发生在 stroke reversal（`φ` 极值点），过渡区占半拍时间的 8%。
...
2. **攻角翻转模型**：在 stroke reversal（φ 极值点）处平滑过渡，使用数值梯度计算 α̇，已修复周期边界连续性。
