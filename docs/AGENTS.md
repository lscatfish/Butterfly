...
| 附加质量力 | `(ρ·π·c_avg²/4)·sign(φ̇)·φ̈·R·r̂₁·sin(α)` ✱ | ≈0（周期对称） |
...
> 4. **上拍升力方向反转**：由于攻角固定，上拍时相对气流方向反转，平动升力方向变为**向下**（负升力）。C_L 在上拍取 -α 时的值实现方向反转。
> 5. **旋转力消失**：固定攻角意味着 `α̇ = 0`，旋转力项为零。
> 6. ✱ 附加质量力采用 Dickinson/Sane 完整公式：`F_AM = (ρπc²/4)·(u·u̇)/|u|·sin(α) = (ρπc²/4)·sign(φ̇)·φ̈·R·r̂₁·sin(α)`；在 φ̇=0 处取极限 `-φ̈·R·r̂₁·sin(α)`（阻力加速度）。
> 7. 实际飞行中三维效应、涡脱落、柔性变形会使真实力显著低于准定常估算（可能低 30–50%）。

### 4. 力计算结果对比

> **注意**：以下数值为 2026-05-29 修正前的历史记录。F_AM 符号修正和 F_peak_total 算法修正后数值将变化，需重新运行验证。

**静态估算**（`analyze_dxf.py`，简化峰值/2）：
| 项目 | 四翅总计 |
|------|---------|
| 重量 | 245 mN |
| 峰值升力 | ~44,500 mN（修正后降低：不再叠加非同时峰值） |
| 时均升力 | ~5,000 mN |
| 时均阻力 | ~4,600 mN |
| 升重比 | ~20 |

**动态分析**（`dynamic_analysis.py`，完整周期积分）：
| 项目 | 数值（固定攻角） | 数值（翻转攻角，旧） |
|------|----------------|-------------------|
| 重量 | 245 mN | 245 mN |
| 峰值升力 | ~78,000 mN（修正后因F_AM符号修正将降低） | ~87,000 mN |
| 时均升力 | ~21,500 mN（修正后将变化） | ~33,800 mN |
| **时均阻力** | **~4,100 mN**（未受F_AM符号影响） | **~7,300 mN** |
| 升重比 | ~88（修正后将降低） | ~138 |
...
3. **F_AM 采用 Dickinson 完整公式**：`F_AM = (ρπc²/4)·sign(φ̇)·φ̈·R·r̂₁·sin(α)`，φ̇=0 处取极限 `-φ̈·R·r̂₁·sin(α)`。
