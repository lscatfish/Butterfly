...
### 4.1 设计参数（机构默认 R=2.25mm）

实际机构设计中摇杆半径 R=2.25mm 是硬约束。将 v6.6 扫描最优的其他参数与 R=2.25, a=6 结合，得到当前**推荐设计参数**：

```python
DESIGN_v66 = {
    "alpha_front_deg": 60,    # 前翅安装角
    "alpha_back_deg": 5,      # 后翅安装角
...
| Peak θ_p | 50.1° |
| n_exceed_90 | 0 (稳定) ✅ |
| mean Fz_world | +566 mN |
| mean Fz_body | +762 mN |

该参数比 v6.5 基线 (L/W=2.505) 提升 15%，已通过 5s 稳定性验证。如需更高 L/W，可增大 R 至 3.0mm（→ L/W=12.625），但需修改机构设计。
